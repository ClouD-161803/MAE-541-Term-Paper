...
We describe the transformations at a high level and emphasize how the Hamiltonian changes at each step. As detailed in \cref{sec:EquilibriumPoints}, the CR3BP satisfies the Hamilton canonical equation \eqref{eq:hamilton_canonical_equation} with Hamiltonian $H$ defined in \eqref{eq:hamiltonian_definition}.
In this section, we adopt the canonical coordinates $\boldsymbol{z} = (\boldsymbol{r}, \boldsymbol{p})$, where $\boldsymbol{p}$ is the canonical momentum (see \cref{sec:EquilibriumPoints} for definitions). The coordinate transformations used below are \emph{symplectic}, meaning that they preserve this canonical Hamiltonian structure.
...
\begin{equation}
    \boldsymbol{\xi}=\boldsymbol{z}-\boldsymbol{\gamma}(t).
\end{equation}
...
\end{equation}
where the quadratic part is now autonomous and $\boldsymbol{S}=-\boldsymbol{J}\boldsymbol{B}$, and $\boldsymbol{J}$ is the symplectic matrix introduced in \cref{sec:EquilibriumPoints}.
...
We draw $100$ samples from this distribution and map each sample to normal form coordinates using the transformations $\boldsymbol{z}\rightarrow\boldsymbol{u}$ described above. 
...
To compare both propagation methods, the analytically propagated normal form trajectories are mapped back to the synodic frame through the inverse transformation $\boldsymbol{u}\rightarrow\boldsymbol{z}$ at the same time instances used by the numerical integrator. 
