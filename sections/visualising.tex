...
%%
%%
    Finally, when visualisation/conceptual understanding of our system is particularly important, we may further reduce our space to a two-dimensional set-up via the projection down to the $x,v_y$ plane. Similar as above, this is thought of as
    \begin{equation*}
         (x,v_x,v_y) \to (x,v_x).
    \end{equation*}
    

    The entirety of this reduction is summed up in the following \cref{fig: reduction tikz}:
    \begin{figure}[ht]
        \centering
    ...
    As established in \cref{sec:EquilibriumPoints}, the equations of motion of the CR$3$BP are Hamiltonian and autonomous. The Hamiltonian (or total energy of the smallest body in the CR$3$BP) is defined in \cref{eq:hamiltonian_definition} for the full six-dimensional state. In this section, however, we consider a reduced-order coordinate vector. For a general point of the form $(x,y,v_x,v_y) \in \R^4$ , the total energy $E = H$ of the system reduces to
    \begin{equation} \label{eq:energy}
    	E = \frac{1}{2} \bigl( v_x^2 + v_y^2 \bigr) + \tilde U(x,y),
    \end{equation}
    where $\tilde U(x,y)$ is the effective potential of the reduced-order CR$3$BP, defined in \cref{sec:EquilibriumPoints}, and simplified by taking $z= v_z = 0$ (the first step in our above reduction).
    % ADJUSTMENT MADE HERE
    % As alluded to at the end of \cref{subsubsec: synodic frame}, it follows from \cite{KoonLoMarsdenRoss2022} that for a general point of the form $(x,y,v_x,v_y) \in \R^4$, the energy thereof is prescribed by the equation
    % %%
    ...
    \begin{equation} \label{eq: energy E2}
        % E_2 = \textcolor{red}{\textbf{WHAT IS THE VALUE (AND UNITS) FOR THE ENERGY AT L2????}},
        E_2 \approx -1.592,
    \end{equation}
    the energy value (in dimensionless CR3BP units) corresponding to the Lagrange point $\mathcal{L}_2$ (see \cref{sec:EquilibriumPoints} for detail). We refer to the region enclosed by this surface as the \textit{accessible region}, as it comprises all states of the form $(x,0,v_x,v_y)$ that have an energy level at or below $E_2$. 
